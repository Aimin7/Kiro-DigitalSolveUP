...
async function getFloodDataByLocation(event, context) {
  ...
  try {
    ...
    // 위치 기반 침수 데이터 조회
    const locationData = await getFloodPointsNearLocation(latitude, longitude, radius, {
      severity,
      alertType,
      limit,
      offset
    ...
    const response = errorResponse('Failed to retrieve location-based flood data', 500, 'LOCATION_FLOOD_ERROR');
    logResponse(response, 'getFloodDataByLocation', Date.now() - startTime);
    return response;
  }
}
...
async function getFloodPointsNearLocation(latitude, longitude, radius, options = {}) {
  try {
    ...
    // 거리 기반 필터링 및 거리 계산
    const nearbyPoints = allData.items.filter(item => {
    ...
    return {
      ...
      summary: {
        ...
        averageDistance: filteredPoints.length > 0 ? filteredPoints.reduce((sum, point) => sum + point.distance, 0) / filteredPoints.length : 0,
    ...
    error('Failed to get flood points near location', err, {
      latitude,
      longitude,
      radius
    });
