...
async function getFloodData(event, context) {
  ...
  try {
    ...
    const response = errorResponse('Failed to retrieve flood data', 500, 'FLOOD_DATA_ERROR');
    logResponse(response, 'getFloodData', Date.now() - startTime);
    return response;
  }
}
...
async function getHanRiverData(event, context) {
  ...
  try {
    ...
    const response = errorResponse('Failed to retrieve Han River data', 500, 'HANRIVER_DATA_ERROR');
    logResponse(response, 'getHanRiverData', Date.now() - startTime);
    return response;
  }
}
  ...
  try {
    ...
    return {
      ...
      summary: {
        ...
        averageDistance: filteredItems.length > 0 ? filteredItems.reduce((sum, item) => sum + item.distance, 0) / filteredItems.length : 0
      }
    };
  } catch (err) {
...
async function getSingleHanRiverAPI(apiType) {
  try {
    ...
    error('Failed to get single Han River API data', err, {
      apiType
    });
    throw err;
  }
...
/******/ 	var __webpack_exports__ = __webpack_require__(5152);
/******/ 	module.exports = __webpack_exports__;
/******/ 	
/******/ })()
;
//# sourceMappingURL=floodHandler.js.map