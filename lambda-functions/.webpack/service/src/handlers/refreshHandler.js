...
class HanRiverAPIService {
  ...
  async requestWithRetry(requestFn, apiType) {
    ...
    for (let attempt = 1; attempt <= this.retryCount; attempt++) {
      try {
        debug(`Han River API request attempt ${attempt}`, {
          apiType
        });
  ...
  async fetchAllData() {
    ...
    // 모든 API를 병렬로 호출
    const [waterLevelResult, realtimeResult, forecastResult] = await Promise.allSettled([this.fetchWaterLevelData(), this.fetchRealtimeData(), this.fetchForecastData()]);
  ...
  processSettledResult(settledResult, apiType) {
    if (settledResult.status === 'fulfilled') {
      return _objectSpread(_objectSpread({}, settledResult.value), {}, {
        apiType,
  ...
  normalizeWaterLevelResponse(rawData) {
    try {
      ...
      error('Failed to normalize water level response', err, {
        rawData
      });
  ...
  normalizeRealtimeResponse(rawData) {
    try {
      ...
      error('Failed to normalize realtime response', err, {
        rawData
      });
  ...
  normalizeForecastResponse(rawData) {
    try {
      ...
      error('Failed to normalize forecast response', err, {
        rawData
      });
  ...
  async checkAPIHealth() {
    ...
    const healthChecks = await Promise.allSettled([this.checkEndpointHealth('waterlevel'), this.checkEndpointHealth('realtime'), this.checkEndpointHealth('forecast')]);
    const results = {
    ...
    results.overall.status = results.overall.healthyCount === 3 ? 'healthy' : results.overall.healthyCount > 0 ? 'degraded' : 'unhealthy';
  ...
  processHealthCheck(settledResult, apiType) {
    ...
    if (settledResult.status === 'fulfilled') {
      return _objectSpread(_objectSpread({}, baseInfo), settledResult.value);
    } else {
  ...
  analyzeDataQuality(apiData) {
    ...
    quality.overall.score = apiScores.length > 0 ? apiScores.reduce((sum, score) => sum + score, 0) / apiScores.length : 0;

    // 전체 이슈 수집
    quality.overall.issues = Object.values(quality.apis).flatMap(api => api.issues || []);
    return quality;
  }
  ...
  detectDataIssues(data, apiType) {
    ...
    if (missingCoords > data.length * 0.1) {
      // 10% 이상
      issues.push({
    ...
    if (apiType === 'waterlevel' || apiType === 'realtime') {
      const invalidLevels = data.filter(item => item.waterLevel !== null && (item.waterLevel < -10 || item.waterLevel > 50)).length;
      if (invalidLevels > 0) {
        issues.push({
          type: 'INVALID_WATER_LEVEL',
          severity: 'high',
          count: invalidLevels
        });
      }
...
class MultiSourceDataService {
    ...
    try {
      ...
      for (const [locationId, locationData] of locationGroups.entries()) {
        try {
          ...
          warn('Failed to create FloodInfo for location', {
            locationId,
            error: err.message
          });
        }
  ...
  groupDataByLocation(normalizedData) {
    ...
    debug('Data grouped by location', {
      ...
      locationsWithMultipleSources: Array.from(locationGroups.values()).filter(group => group.sources.size > 1).length
    });
  ...
  findNearbyLocation(item, locationGroups) {
    for (const [locationId, group] of locationGroups.entries()) {
      const distance = FloodInfo.calculateDistance(item.latitude, item.longitude, group.latitude, group.longitude);
  ...
  async createFloodInfoFromLocationData(locationId, locationData) {
    try {
      ...
      // 추가 메타데이터 설정
      floodInfo.metadata = _objectSpread(_objectSpread({}, floodInfo.metadata), {}, {
      ...
      error('Failed to create FloodInfo from location data', err, {
        locationId
      });
      throw err;
    }
  ...
  async getMultiSourceDataByLocation(locationId) {
    try {
      debug('Getting multi-source data by location', {
        locationId
      });
      ...
      locationData.items.forEach(item => {
        ...
        if (item.apiType === 'waterlevel' && item.originalData) {
          if (!multiSourceData.waterLevelData || new Date(item.timestamp) > new Date(multiSourceData.waterLevelData.timestamp)) {
            multiSourceData.waterLevelData = _objectSpread(_objectSpread({}, item.originalData), {}, {
              timestamp: item.timestamp,
              normalizedAt: item.normalizedAt
            });
          }
        } else if (item.apiType === 'realtime' && item.originalData) {
          if (!multiSourceData.realtimeData || new Date(item.timestamp) > new Date(multiSourceData.realtimeData.timestamp)) {
            multiSourceData.realtimeData = _objectSpread(_objectSpread({}, item.originalData), {}, {
              timestamp: item.timestamp,
              normalizedAt: item.normalizedAt
            });
          }
        } else if (item.apiType === 'forecast' && item.originalData) {
          if (!multiSourceData.forecastData || new Date(item.timestamp) > new Date(multiSourceData.forecastData.timestamp)) {
            multiSourceData.forecastData = _objectSpread(_objectSpread({}, item.originalData), {}, {
              timestamp: item.timestamp,
              normalizedAt: item.normalizedAt
            });
          }
        ...
        // 최신 업데이트 시간
        if (!multiSourceData.lastUpdated || new Date(item.timestamp) > new Date(multiSourceData.lastUpdated)) {
      ...
      error('Failed to get multi-source data by location', err, {
        locationId
      });
  ...
  async getMultiSourceDataNearby(latitude, longitude, radiusMeters = 5000) {
    try {
      debug('Getting nearby multi-source data', {
        latitude,
        longitude,
        radiusMeters
      });
      ...
      const nearbyItems = allData.items.filter(item => {
        ...
        const distance = FloodInfo.calculateDistance(latitude, longitude, item.latitude, item.longitude);
      ...
      for (const [locationId, items] of locationGroups.entries()) {
        try {
          ...
          if (multiSourceResult.found) {
            // 거리 정보 추가
            const distance = FloodInfo.calculateDistance(latitude, longitude, multiSourceResult.data.latitude, multiSourceResult.data.longitude);
            nearbyMultiSourceData.push(_objectSpread(_objectSpread({}, multiSourceResult.data), {}, {
              distance
            }));
          }
        } catch (err) {
          warn('Failed to get multi-source data for nearby location', {
            locationId,
            error: err.message
          });
        }
      ...
      error('Failed to get nearby multi-source data', err, {
        latitude,
        longitude,
        radiusMeters
      });
  ...
  async updateMultiSourceData(locationId, newData) {
    try {
      debug('Updating multi-source data', {
        locationId,
      ...
      if (!existingData.found) {
        ...
        info('New multi-source data created', {
          locationId,
          apiType: newData.apiType
        });
        return {
      ...
      error('Failed to update multi-source data', err, {
        locationId
      });
  ...
  async mergeNewDataWithExisting(existingData, newData) {
    try {
      ...
      if (existingApiData && new Date(existingApiData.timestamp) >= new Date(newData.timestamp)) {
  ...
  async getMultiSourceStatistics() {
    try {
      ...
      stats.averageSourcesPerLocation = stats.locationCount > 0 ? totalSources / stats.locationCount : 0;
  ...
  async cleanupOldData(maxAgeHours = 24) {
    try {
      ...
      error('Failed to cleanup old data', err, {
        maxAgeHours
      });
...
class DataNormalizationService {
  ...
  normalizeWaterLevelData(rawData) {
    try {
      debug('Normalizing water level data', {
      ...
      error('Failed to normalize water level data', err, {
  ...
  normalizeRealtimeData(rawData) {
    try {
      debug('Normalizing realtime data', {
        count: rawData?.length
      });
      if (!Array.isArray(rawData)) {
      ...
      error('Failed to normalize realtime data', err, {
        rawDataCount: rawData?.length
      });
      throw new Error(`Realtime data normalization failed: ${err.message}`);
    }
  ...
  normalizeForecastData(rawData) {
    try {
      debug('Normalizing forecast data', {
        count: rawData?.length
      });
      if (!Array.isArray(rawData)) {
      ...
      const normalizedData = rawData.map(item => {
        const coordinates = this.normalizeCoordinates(item.coordinates) || this.estimateCoordinatesFromRegion(item.region);
        return {
          id: uuidv4(),
          locationId: coordinates ? this.generateLocationId(coordinates.latitude, coordinates.longitude) : null,
          apiType: 'forecast',
      ...
      error('Failed to normalize forecast data', err, {
        rawDataCount: rawData?.length
      });
      throw new Error(`Forecast data normalization failed: ${err.message}`);
    }
  ...
  isValidCoordinate(latitude, longitude) {
    ...
    if (latitude < 33 || latitude > 39 || longitude < 124 || longitude > 132) {
      warn('Coordinates outside Korea region', {
        latitude,
        longitude
      });
    }
  ...
  estimateCoordinatesFromRegion(region) {
    ...
    for (const [key, coords] of Object.entries(regionCoordinates)) {
      if (region.includes(key)) {
        debug('Estimated coordinates from region', {
          region,
          key,
          coords
        });
    ...
    // 기본값: 서울시청 좌표
    warn('Could not estimate coordinates from region, using default', {
      region
    });
    return {
      latitude: 37.5665,
  ...
  validateDataQuality(normalizedData) {
    ...
    validation.qualityScore = validation.totalItems > 0 ? validation.validItems / validation.totalItems : 0;
  ...
  generateNormalizationStats(normalizedResults) {
    ...
    if (normalizedResults.metadata) {
      ...
      stats.overall.successRate = stats.overall.totalOriginalItems > 0 ? stats.overall.totalNormalizedItems / stats.overall.totalOriginalItems : 0;
    }

    // 전체 좌표 완성도
    const totalWithCoords = Object.values(stats.apiStats).reduce((sum, api) => sum + api.normalizedCount * api.coordinateCompleteness, 0);
    stats.overall.coordinateCompleteness = stats.overall.totalNormalizedItems > 0 ? totalWithCoords / stats.overall.totalNormalizedItems : 0;
    return stats;
  }
  ...
  calculateAveragePrecision(data) {
    if (!data || data.length === 0) return 0;
    const precisions = data.filter(item => item.latitude && item.longitude).map(item => {
    ...
    return precisions.length > 0 ? precisions.reduce((sum, p) => sum + p, 0) / precisions.length : 0;
  }
}
...
const HanRiverAPIService = __webpack_require__(175);
const DataNormalizationService = __webpack_require__(3263);
const MultiSourceDataService = __webpack_require__(1736);
...
exports.refreshFloodData = async (event, context) => {
  try {
    ...
    for (const apiType of apiTypes) {
      try {
        debug(`Refreshing ${apiType} data`, {
          apiType,
          region
        });
        const apiResult = await refreshAPIData(apiType, hanRiverService, normalizationService, multiSourceService, {
          forceRefresh,
          region
        });
...
async function refreshAPIData(apiType, hanRiverService, normalizationService, multiSourceService, options = {}) {
  ...
  try {
    debug(`Starting ${apiType} API data refresh`, {
      apiType,
      options
    });
    let rawData = [];
...
async function storeAPIData(apiType, normalizedData, multiSourceService, forceRefresh) {
  try {
    ...
    switch (apiType) {
      case 'waterlevel':
        storeResult = await multiSourceService.storeMultiSourceData(normalizedData, [], [] // 수위관측소 데이터만
        );
        break;
      case 'realtime':
        storeResult = await multiSourceService.storeMultiSourceData([], normalizedData, [] // 실시간 데이터만
        );
        break;
      case 'forecast':
        storeResult = await multiSourceService.storeMultiSourceData([], [], normalizedData // 예보 데이터만
        );
        break;
      default:
