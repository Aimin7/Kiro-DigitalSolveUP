...
class NaverGeocodingService {
  ...
  async requestWithRetry(requestFn, operation) {
    ...
    for (let attempt = 1; attempt <= this.retryCount; attempt++) {
      try {
        debug(`Naver API request attempt ${attempt}`, {
          operation
        });
        const startTime = Date.now();
        const response = await requestFn();
  ...
  async addressToCoordinates(address) {
    try {
      debug('Converting address to coordinates', {
        address
      });
      ...
      error('Failed to convert address to coordinates', err, {
        address
      });
      return {
        success: false,
  ...
  async coordinatesToAddress(latitude, longitude) {
    try {
      debug('Converting coordinates to address', {
        latitude,
        longitude
      });
      ...
      error('Failed to convert coordinates to address', err, {
        latitude,
        longitude
      });
      return {
  ...
  async batchAddressToCoordinates(addresses) {
    try {
      debug('Starting batch geocoding', {
...
const NaverGeocodingService = __webpack_require__(687);
...
async function getSafeRoute(event, context) {
  ...
  try {
    ...
    if (!safeRouteResult.success) {
      const response = errorResponse(safeRouteResult.error.message, 500, safeRouteResult.error.code);
      logResponse(response, 'getSafeRoute', Date.now() - startTime);
      return response;
    }
    const responseData = _objectSpread(_objectSpread({}, safeRouteResult.data), {}, {
      avoidanceRadius,
    ...
    const response = errorResponse('Failed to calculate safe route', 500, 'SAFE_ROUTE_ERROR');
    logResponse(response, 'getSafeRoute', Date.now() - startTime);
    return response;
  }
}
...
async function geocodeAddress(event, context) {
  ...
  try {
    ...
    if (operation === 'addressToCoordinates') {
      ...
      // 좌표 -> 주소 변환
      geocodingResult = await geocodingService.coordinatesToAddress(coordinates.latitude, coordinates.longitude);
    } else {
    ...
    if (!geocodingResult.success) {
      const response = errorResponse(geocodingResult.error.message, 400, geocodingResult.error.code);
      logResponse(response, 'geocodeAddress', Date.now() - startTime);
      return response;
    }
    ...
    const response = errorResponse('Failed to process geocoding request', 500, 'GEOCODING_ERROR');
    logResponse(response, 'geocodeAddress', Date.now() - startTime);
    return response;
  }
}
...
async function batchGeocode(event, context) {
  ...
  try {
    ...
    if (!batchResult.success) {
      const response = errorResponse('Batch geocoding failed', 500, 'BATCH_GEOCODING_ERROR');
      logResponse(response, 'batchGeocode', Date.now() - startTime);
      return response;
    }
    ...
    const response = errorResponse('Failed to process batch geocoding request', 500, 'BATCH_GEOCODING_ERROR');
    logResponse(response, 'batchGeocode', Date.now() - startTime);
    return response;
  }
}
...
async function checkNaverServiceHealth(event, context) {
  ...
  try {
    ...
    const [geocodingHealth, directionsHealth] = await Promise.allSettled([geocodingService.checkServiceHealth(), directionsService.checkServiceHealth()]);
    const healthStatus = {
      overall: 'healthy',
      services: {
        geocoding: geocodingHealth.status === 'fulfilled' ? geocodingHealth.value : {
          healthy: false,
          error: geocodingHealth.reason.message
        },
        directions: directionsHealth.status === 'fulfilled' ? directionsHealth.value : {
          healthy: false,
          error: directionsHealth.reason.message
        }
      },
      timestamp: new Date().toISOString()
    };
    ...
    const response = errorResponse('Failed to check service health', 500, 'HEALTH_CHECK_ERROR');
    logResponse(response, 'checkNaverServiceHealth', Date.now() - startTime);
    return response;
  }
}
...
/******/ 	var __webpack_exports__ = __webpack_require__(5122);
/******/ 	module.exports = __webpack_exports__;
/******/ 	
/******/ })()
;
//# sourceMappingURL=naverHandler.js.map