...
async function getMultiSourceData(event, context) {
  ...
  try {
    ...
    if (validationResult.error) {
      const response = validationError('Invalid query parameters', validationResult.error.details);
      logResponse(response, 'getMultiSourceData', Date.now() - startTime);
      return response;
    }
    const {
    ...
    // 응답 데이터 포맷팅
    const responseData = formatMultiSourceResponse(multiSourceResult.data, {
      includeInactive,
      format
    });
    ...
    const response = errorResponse('Failed to retrieve multi-source data', 500, 'MULTI_SOURCE_ERROR');
    logResponse(response, 'getMultiSourceData', Date.now() - startTime);
    return response;
  }
}
...
async function getMultiSourceDataList(event, context) {
  ...
  try {
    ...
    if (latitude && longitude) {
      multiSourceDataList = await multiSourceService.getMultiSourceDataNearby(latitude, longitude, radius);
    } else {
      // 전체 조회
    ...
    if (minSources > 1) {
      filteredData = filteredData.filter(item => item.availableAPIs && item.availableAPIs.length >= minSources);
    }
    if (severity) {
    ...
    const responseData = {
      ...
      summary: {
        ...
        multiSourceCount: filteredData.filter(item => item.availableAPIs && item.availableAPIs.length > 1).length,
    ...
    const response = errorResponse('Failed to retrieve multi-source data list', 500, 'MULTI_SOURCE_LIST_ERROR');
    logResponse(response, 'getMultiSourceDataList', Date.now() - startTime);
    return response;
  }
}
...
async function getMultiSourceStatistics(event, context) {
  ...
  try {
    ...
    const response = errorResponse('Failed to retrieve multi-source statistics', 500, 'STATISTICS_ERROR');
    logResponse(response, 'getMultiSourceStatistics', Date.now() - startTime);
    return response;
  }
}
...
function formatMultiSourceResponse(multiSourceData, options = {}) {
  ...
  // API별 데이터 섹션
  const apiSections = {
    waterLevelData: formatAPISection(multiSourceData.waterLevelData, 'waterlevel', {
      includeInactive,
      format
    }),
    realtimeData: formatAPISection(multiSourceData.realtimeData, 'realtime', {
      includeInactive,
      format
    }),
    forecastData: formatAPISection(multiSourceData.forecastData, 'forecast', {
      includeInactive,
      format
    })
  };
  return _objectSpread(_objectSpread(_objectSpread({}, baseResponse), apiSections), {}, {
    metadata: {
...
function getMaxSeverityFromMultiSource(multiSourceItem) {
  ...
  if (multiSourceItem.waterLevelData) {
    ...
    if (status === 'danger') severities.push('high');else if (status === 'alert') severities.push('medium');else severities.push('low');
  }
  if (multiSourceItem.forecastData) {
    const alertType = multiSourceItem.forecastData.alertType;
    if (alertType === '특보') severities.push('high');else if (alertType === '경보') severities.push('medium');else severities.push('low');
  }
...
/******/ 	var __webpack_exports__ = __webpack_require__(9176);
/******/ 	module.exports = __webpack_exports__;
/******/ 	
/******/ })()
;
//# sourceMappingURL=multiSourceHandler.js.map