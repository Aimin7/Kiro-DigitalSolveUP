...
const Assert = __webpack_require__(1279);

const Any = __webpack_require__(341);
const Common = __webpack_require__(8620);
...
const Base = __webpack_require__(2456);
const Common = __webpack_require__(8620);
const Messages = __webpack_require__(8693);
...
const Assert = __webpack_require__(1279);
const Clone = __webpack_require__(2596);
const EscapeHtml = __webpack_require__(4399);
const Formula = __webpack_require__(7622);

const Common = __webpack_require__(8620);
const Errors = __webpack_require__(6770);
const Ref = __webpack_require__(8988);
...
class NaverDirectionsService {
  ...
  async requestWithRetry(requestFn, operation) {
    ...
    for (let attempt = 1; attempt <= this.retryCount; attempt++) {
      try {
        debug(`Naver Directions API request attempt ${attempt}`, {
          operation
        });
    ...
    try {
      ...
      return this.requestWithRetry(async () => {
        ...
        if (options.waypoints && options.waypoints.length > 0) {
          const waypointStr = options.waypoints.map(wp => `${wp.longitude},${wp.latitude}`).join('|');
      ...
      error('Failed to calculate route', err, {
        start,
        goal
      });
      return {
  ...
  async calculateSafeRoute(start, goal, floodAreas) {
    try {
      ...
      const proximityCheck = await this.checkRouteProximity(originalRoute.data.route.traoptimal?.path || [], floodAreas, this.avoidanceRadius);
      ...
      error('Failed to calculate safe route', err, {
        start,
        goal
      });
    ...
    try {
      ...
      if (!avoidanceRoute.success) {
        ...
        for (const option of alternativeOptions) {
          const altRoute = await this.calculateRoute(start, goal, {
            option
          });
          if (altRoute.success) {
            const altProximityCheck = await this.checkRouteProximity(altRoute.data.route[option]?.path || [], avoidAreas, this.avoidanceRadius);
  ...
  generateAvoidanceWaypoints(start, goal, avoidAreas) {
    ...
    avoidAreas.forEach(area => {
      const intersection = this.findLineCircleIntersection(routeLine, {
        center: area,
        radius: area.radius || this.avoidanceRadius
      });
      if (intersection.intersects) {
        // 회피 지점 생성 (원의 접선 방향으로)
        const avoidancePoint = this.generateAvoidancePoint(area, routeLine, this.avoidanceRadius * 1.2 // 여유 거리
        );
        if (avoidancePoint) {
    ...
    debug('Generated avoidance waypoints', {
  ...
  findLineCircleIntersection(line, circle) {
    ...
    const startDist = this.calculateDistance(line.start.latitude, line.start.longitude, circle.center.latitude, circle.center.longitude);
    const endDist = this.calculateDistance(line.end.latitude, line.end.longitude, circle.center.latitude, circle.center.longitude);
    const intersects = startDist <= circle.radius || endDist <= circle.radius || startDist > circle.radius && endDist > circle.radius && Math.min(startDist, endDist) <= circle.radius;
    return {
      intersects,
  ...
  generateAvoidancePoint(avoidArea, routeLine, avoidanceDistance) {
    try {
      ...
      warn('Failed to generate avoidance point', {
        avoidArea,
        error: err.message
      });
      return null;
      ...
      for (const floodPoint of floodPoints) {
        ...
        for (let i = 0; i < routePath.length - 1; i++) {
          ...
          const distance = this.pointToLineDistance({
            latitude: floodPoint.latitude,
            longitude: floodPoint.longitude
          }, {
  ...
  pointToLineDistance(point, lineStart, lineEnd) {
    // 선분의 길이가 0인 경우
    const lineLength = this.calculateDistance(lineStart.latitude, lineStart.longitude, lineEnd.latitude, lineEnd.longitude);
    if (lineLength === 0) {
      return this.calculateDistance(point.latitude, point.longitude, lineStart.latitude, lineStart.longitude);
    }

    // 점에서 선분으로의 투영 계산
    const t = Math.max(0, Math.min(1, ((point.latitude - lineStart.latitude) * (lineEnd.latitude - lineStart.latitude) + (point.longitude - lineStart.longitude) * (lineEnd.longitude - lineStart.longitude)) / lineLength ** 2));
    ...
    // 점과 투영점 간 거리 반환
    return this.calculateDistance(point.latitude, point.longitude, projectionLat, projectionLng);
  }
  ...
  isValidCoordinate(latitude, longitude) {
    return typeof latitude === 'number' && typeof longitude === 'number' && !isNaN(latitude) && !isNaN(longitude) && latitude >= -90 && latitude <= 90 && longitude >= -180 && longitude <= 180;
  }
...
var path = __webpack_require__(4547);
...
const AssertError = __webpack_require__(1595);
...
function v1ToV6(uuid) {
  const v1Bytes = typeof uuid === 'string' ? esm_node_parse(uuid) : uuid;
...
function md5(bytes) {
  ...
  return external_node_crypto_default().createHash('md5').update(bytes).digest();
}
...
function sha1(bytes) {
  ...
  return external_node_crypto_default().createHash('sha1').update(bytes).digest();
}
...
function v6ToV1(uuid) {
  const v6Bytes = typeof uuid === 'string' ? esm_node_parse(uuid) : uuid;
...
/* harmony default export */ const esm_node_v7 = (v7);
...
const Messages = __webpack_require__(8693);
const Ref = __webpack_require__(8988);
const Template = __webpack_require__(551);
...
internals.validate = function (joi, desc) {
    ...
    Schemas = Schemas || __webpack_require__(6297);
...
const Stringify = __webpack_require__(3744);
...
const Assert = __webpack_require__(1279);
const DeepEqual = __webpack_require__(5415);
const Reach = __webpack_require__(7450);

const Any = __webpack_require__(341);
const Common = __webpack_require__(8620);
const Compile = __webpack_require__(7276);
...
const DeepEqual = __webpack_require__(5415);
const Pinpoint = __webpack_require__(6247);
...
const Errors = __webpack_require__(2033);
...
const Clone = __webpack_require__(2596);
const DeepEqual = __webpack_require__(5415);
const Merge = __webpack_require__(4413);

const Cache = __webpack_require__(2155);
const Common = __webpack_require__(8620);
const Compile = __webpack_require__(7276);
const Errors = __webpack_require__(6770);
const Extend = __webpack_require__(3813);
const Manifest = __webpack_require__(1444);
const Messages = __webpack_require__(8693);
const Modify = __webpack_require__(8787);
const Ref = __webpack_require__(8988);
const Trace = __webpack_require__(2136);
const Validator = __webpack_require__(4323);
const Values = __webpack_require__(3507);
...
const Reach = __webpack_require__(7450);
const Types = __webpack_require__(7976);
const Utils = __webpack_require__(6292);
...
const Keys = __webpack_require__(4923);
...
const Assert = __webpack_require__(1279);
const EscapeRegex = __webpack_require__(2415);
...
const ProximityCheckService = __webpack_require__(8505);
const NaverDirectionsService = __webpack_require__(680);
const DynamoDBService = __webpack_require__(7320);
const Joi = __webpack_require__(6461);
...
async function checkProximity(event, context) {
  ...
  try {
    ...
    if (validationResult.error) {
      const response = validationError('Invalid request body', validationResult.error.details);
    ...
    // 경로 근접성 검사 실행
    const proximityResult = await proximityService.checkRouteProximity(routePath, floodPoints, proximityRadius);

    // 응답 데이터 구성
    const responseData = _objectSpread(_objectSpread({}, proximityResult), {}, {
      routeInfo: {
    ...
    const response = errorResponse('Failed to check route proximity', 500, 'PROXIMITY_CHECK_ERROR');
    logResponse(response, 'checkProximity', Date.now() - startTime);
    return response;
  }
}
...
async function getAlternativeRoute(event, context) {
  ...
  try {
    ...
    // 대체 경로 계산
    const alternativeResult = await directionsService.calculateRouteWithAvoidance(start, goal, floodAreas);
    if (!alternativeResult.success) {
      const response = errorResponse(alternativeResult.error.message, 500, alternativeResult.error.code);
      logResponse(response, 'getAlternativeRoute', Date.now() - startTime);
      return response;
    }
    ...
    const response = errorResponse('Failed to calculate alternative route', 500, 'ALTERNATIVE_ROUTE_ERROR');
    logResponse(response, 'getAlternativeRoute', Date.now() - startTime);
    return response;
  }
}
...
async function analyzeRegionalProximity(event, context) {
  ...
  try {
    ...
    // 지역별 근접성 분석 실행
    const analysisResult = await proximityService.analyzeRegionalProximity(bounds, proximityRadius);
    ...
    const response = errorResponse('Failed to analyze regional proximity', 500, 'REGIONAL_ANALYSIS_ERROR');
    logResponse(response, 'analyzeRegionalProximity', Date.now() - startTime);
    return response;
  }
}
...
function validateProximityRequest(requestBody) {
  const schema = Joi.object({
    routePath: Joi.array().items(Joi.array().items(Joi.number()).length(2) // [lng, lat] 형식
    ).min(2).required(),
    proximityRadius: Joi.number().min(100).max(5000).default(1500),
    // 100m ~ 5km
...
function validateAlternativeRouteRequest(requestBody) {
  const schema = Joi.object({
    ...
    avoidanceRadius: Joi.number().min(500).max(5000).default(1500),
    // 0.5km ~ 5km
...
function calculateRouteDistance(routePath) {
  ...
  for (let i = 0; i < routePath.length - 1; i++) {
    ...
    totalDistance += calculateDistance(start[1], start[0],
    // lat, lng
    end[1], end[0]);
  }
...
class FloodInfo {
  ...
  merge(other) {
    ...
    // 더 최신 데이터로 업데이트
    const merged = new FloodInfo(_objectSpread(_objectSpread(_objectSpread({}, this.toObject()), other.toObject()), {}, {
  ...
  getDistanceFrom(latitude, longitude) {
    return FloodInfo.calculateDistance(this.latitude, this.longitude, latitude, longitude);
  }
...
const Assert = __webpack_require__(1279);
const Clone = __webpack_require__(2596);
const Ignore = __webpack_require__(83);
const Reach = __webpack_require__(7450);

const Common = __webpack_require__(8620);
const Errors = __webpack_require__(6770);
const State = __webpack_require__(8816);
...
const ApplyToDefaults = __webpack_require__(4418);
const Assert = __webpack_require__(1279);
const Clone = __webpack_require__(2596);
const Topo = __webpack_require__(6312);
...
var extname = (__webpack_require__(4547).extname)
...
const internals = {
    types: {
        alternatives: __webpack_require__(6991),
        any: __webpack_require__(341),
        array: __webpack_require__(1854),
        boolean: __webpack_require__(6283),
        date: __webpack_require__(9499),
        function: __webpack_require__(2607),
        link: __webpack_require__(5975),
        number: __webpack_require__(2850),
        object: __webpack_require__(4884),
        string: __webpack_require__(6498),
        symbol: __webpack_require__(9099)
    },
    aliases: {
        alt: 'alternatives',
        bool: 'boolean',
...
if (Buffer) {                                                           // $lab:coverage:ignore$
    internals.types.binary = __webpack_require__(198);
}
...
const Assert = __webpack_require__(1279);
const Domain = __webpack_require__(2378);
const Email = __webpack_require__(9604);
const Ip = __webpack_require__(9731);
const EscapeRegex = __webpack_require__(2415);
const Tlds = __webpack_require__(1471);
const Uri = __webpack_require__(3374);
...
const Annotate = __webpack_require__(8805);
const Common = __webpack_require__(8620);
const Template = __webpack_require__(551);
...
if ( false || process.env.IS_OFFLINE) {
...
class DynamoDBService {
  ...
  async getItem(id) {
    try {
      ...
      info('DynamoDB item retrieved successfully', {
        id
      });
      return result.Item;
    } catch (err) {
    ...
    try {
      ...
      if (conditionExpression) {
        ...
        if (conditionExpression.values) {
          params.ExpressionAttributeValues = _objectSpread(_objectSpread({}, params.ExpressionAttributeValues), conditionExpression.values);
        }
        if (conditionExpression.names) {
          params.ExpressionAttributeNames = _objectSpread(_objectSpread({}, params.ExpressionAttributeNames), conditionExpression.names);
        }
      }
      ...
      if (err.code === 'ConditionalCheckFailedException') {
        info('DynamoDB conditional update failed - condition not met', {
          id
        });
        return {
          success: false,
...
const FloodInfo = __webpack_require__(3737);
const DynamoDBService = __webpack_require__(7320);
...
class ProximityCheckService {
  ...
  async getFloodPointsInBounds(bounds) {
    try {
      ...
      // 경계 박스 내 필터링
      const floodPoints = result.items.map(item => FloodInfo.fromDynamoDBItem(item)).filter(floodInfo => floodInfo.latitude >= bounds.south && floodInfo.latitude <= bounds.north && floodInfo.longitude >= bounds.west && floodInfo.longitude <= bounds.east);
      return floodPoints;
    } catch (err) {
      warn('Failed to get flood points in bounds', {
        error: err.message,
        bounds
      });
      return [];
  ...
  calculateMinDistanceToRoute(routePath, floodPoint) {
    ...
    for (let i = 0; i < routePath.length - 1; i++) {
      const segmentStart = {
        latitude: routePath[i][1],
        longitude: routePath[i][0]
      };
      const segmentEnd = {
        latitude: routePath[i + 1][1],
        longitude: routePath[i + 1][0]
      };
  ...
  calculateDistanceToSegment(segmentStart, segmentEnd, point) {
    // 선분의 길이
    const segmentLength = FloodInfo.calculateDistance(segmentStart.latitude, segmentStart.longitude, segmentEnd.latitude, segmentEnd.longitude);
    if (segmentLength === 0) {
      // 선분이 점인 경우
      return FloodInfo.calculateDistance(segmentStart.latitude, segmentStart.longitude, point.latitude, point.longitude);
    }
    ...
    // 거리 계산
    return FloodInfo.calculateDistance(closestLat, closestLng, point.latitude, point.longitude);
  }
  ...
  calculateRouteDistance(routePath) {
    ...
    for (let i = 0; i < routePath.length - 1; i++) {
      const start = routePath[i];
      const end = routePath[i + 1];
      totalDistance += FloodInfo.calculateDistance(start[1], start[0], end[1], end[0]);
    }
    return totalDistance;
  }
  ...
  determineAlertLevel(distance, proximityRadius, severity) {
    ...
    // 거리 기반 기본 레벨
    let baseLevel;
    if (distance <= proximityRadius * 0.3) {
      // 30% 이내
      baseLevel = 'high';
    } else if (distance <= proximityRadius * 0.7) {
      // 70% 이내
      baseLevel = 'medium';
    } else {
      baseLevel = 'low';
  ...
  identifyRiskSegments(routePath, alertPoints) {
    ...
    alertPoints.forEach(alertPoint => {
      ...
      for (let i = 0; i < routePath.length - 1; i++) {
        ...
        const distance = this.calculateDistanceToSegment(segmentStart, segmentEnd, floodPoint);
        if (distance <= alertPoint.minDistance * 1.1) {
          // 10% 여유
          riskSegments.push({
            segmentIndex: i,
  ...
  calculateRouteSafetyScore(proximityResult) {
    ...
    const {
      high,
      medium
    } = routeAnalysis.severityDistribution;
  ...
  startProximityMonitoring(routePath, callback, options = {}) {
    ...
    const monitor = async () => {
      ...
      try {
        ...
        if (result.hasProximityAlert || lastCheckTime && result.timestamp !== lastCheckTime) {
          callback(result);
        }
...
exports.preferences = function (target, source) {
    ...
    Messages = Messages || __webpack_require__(8693);
...
exports.Manager = class {
    ...
    register(source, target) {
        ...
        Template = Template || __webpack_require__(551);
...
/******/ 	var __webpack_exports__ = __webpack_require__(3459);
/******/ 	module.exports = __webpack_exports__;
/******/ 	
/******/ })()
;
//# sourceMappingURL=proximityHandler.js.map